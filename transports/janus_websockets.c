 ...
 * Janus API, using the lwss library (http://lwss.org).
...
#define JANUS_WEBSOCKETS_DESCRIPTION		"This transport plugin adds WebSockets support to the Janus API via lwss."
...
/* lwss sessions that have been closed */
...
int janus_websockets_init(janus_transport_callbacks *callback, const char *config_path) {
	...
	if(config != NULL) {
		...
		JANUS_LOG(LOG_VERB, "lwss logging: %d\n", ws_log_level);
...
void *janus_websockets_thread(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		/* lwss is single thread, we cycle through events here */
